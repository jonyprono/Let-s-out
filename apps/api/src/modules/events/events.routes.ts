...
export default async function eventsRoutes(app: FastifyInstance) {
  ...
  app.get('/recommended', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    const events = await app.prisma.event.findMany({
      ...
      include: {
        ...
        _count: { select: { bookings: true, comments: true, reactions: true } },
        ...(sub ? { reactions: { where: { userId: sub } } } : {}),
        payoutRequests: true,
      },
      orderBy: { startAt: 'asc' },
      take: 10,
  ...
  app.get('/', { preHandler: [app.optionalAuthenticate] }, async (req, reply) => {
    ...
    const events = await app.prisma.event.findMany({
      ...
      include: {
        ...
        _count: { select: { bookings: true, comments: true, reactions: true } },
        ...(currentUser ? { reactions: { where: { userId: currentUser } } } : {}),
      },
      orderBy: { startAt: 'asc' },
      take: Number(limit),
      skip: Number(offset),
  ...
  app.delete('/:id/comments/:commentId', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    if (!comment || comment.eventId !== eventId) return reply.code(404).send({ error: 'Commentaire introuvable' })
