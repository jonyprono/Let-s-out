  ...
  ({ className, provider, children, ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          "flex h-[44px] w-full items-center justify-center gap-[var(--spacing-150)] rounded-[var(--radius-large)] border border-[var(--border-default)] bg-[var(--color-background-primary)] px-[var(--spacing-200)] transition-colors hover:bg-[var(--color-background-secondary)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--border-brand-primary)] disabled:pointer-events-none disabled:opacity-50",
          className
        )}
        {...props}
      >
        ...
        <span className="font-poppins text-[var(--font-size-body-medium)] font-medium text-[var(--color-text-secondary)]">
          {children}
        </span>
      </button>
    )
