        ...
        {imageUrl ? (
          <div className="overflow-hidden rounded-[var(--radius-large)] border border-[var(--border-secondary)] relative group flex flex-col">
            <img 
              src={imageUrl} 
              alt="attachment" 
              className={cn("max-h-[200px] w-auto object-cover transition-opacity", onImageClick && "cursor-pointer hover:opacity-90 active:opacity-75", status === 'sending' && "opacity-60 grayscale-[30%]")} 
              onClick={(e) => {
                if (onImageClick) {
                  e.stopPropagation();
                  onImageClick();
            ...
            {status === 'sending' && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/10 z-10 pointer-events-none">
                <svg className="animate-spin h-8 w-8 text-white drop-shadow-md" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
