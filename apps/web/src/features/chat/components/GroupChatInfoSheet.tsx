...
export function GroupChatInfoSheet({ conversation, event, onClose, onInvite, onContribute }: GroupChatInfoSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#FAFAFA] flex flex-col pt-safe-6 animate-in slide-in-from-right duration-300 overflow-y-auto">
      {/* Header */}
      <div className="flex items-center px-4 py-2 sticky top-0 bg-[#FAFAFA] z-10">
      ...
      <div className="flex flex-col items-center px-4 pb-10 gap-6 max-w-md mx-auto w-full">
            ...
            {conversation.members.map((member) => (
              <div key={member.userId} className="flex flex-row items-center gap-[6px] w-full h-[32px]">
                ...
                <span className="flex-1 font-['Inter_Display'] font-normal text-[12px] leading-[16px] text-[#1B1818] truncate">
                  {member.user?.profile?.displayName} {member.userId === user?.id ? '(Moi)' : ''}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-1 w-full mt-2">
          ...
          <div className="flex flex-col gap-[2px] w-full">
            {event && (
              <button onClick={() => navigate(`/events/${event.id}`)} className="flex flex-row items-center p-2 pl-0 gap-2 w-full h-[36px] rounded-[8px] active:bg-gray-100 transition-colors">
                <div className="w-5 h-5 flex items-center justify-center">
                  <CalendarDays className="w-5 h-5 text-[#737373]" strokeWidth={1.25} />
                </div>
                <span className="font-poppins font-medium text-[14px] leading-[20px] text-[#525252]">Voir l'événement</span>
              </button>
            )}
            
            <button onClick={onInvite} className="flex flex-row items-center p-2 pl-0 gap-2 w-full h-[36px] rounded-[8px] active:bg-gray-100 transition-colors">
              <div className="w-5 h-5 flex items-center justify-center">
                <UserPlus className="w-5 h-5 text-[#737373]" strokeWidth={1.25} />
              </div>
              <span className="font-poppins font-medium text-[14px] leading-[20px] text-[#525252]">Inviter des amis</span>
            </button>

            {poolStats && (
              <button onClick={onContribute} className="flex flex-row items-center p-2 pl-0 gap-2 w-full h-[36px] rounded-[8px] active:bg-gray-100 transition-colors">
                <div className="w-5 h-5 flex items-center justify-center">
                  <Wallet className="w-5 h-5 text-[#737373]" strokeWidth={1.25} />
                </div>
                <span className="font-poppins font-medium text-[14px] leading-[20px] text-[#525252]">Voir la cagnotte</span>
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 w-full mt-2">
          ...
          <div className="flex flex-col gap-[2px] w-full">
            <button className="flex flex-row items-center p-2 pl-0 gap-2 w-full h-[36px] rounded-[8px] active:bg-gray-100 transition-colors">
              <div className="w-5 h-5 flex items-center justify-center">
                <BellOff className="w-5 h-5 text-[#737373]" strokeWidth={1.25} />
              </div>
              <span className="font-poppins font-medium text-[14px] leading-[20px] text-[#525252]">Mettre en sourdine</span>
            </button>
            ...
            <button className="flex flex-row items-center p-2 pl-0 gap-2 w-full h-[36px] rounded-[8px] active:bg-gray-100 transition-colors">
              ...
              <span className="font-poppins font-medium text-[14px] leading-[20px] text-[#525252]">Signaler</span>
            </button>
            ...
            <button className="flex flex-row items-center p-2 pl-0 gap-2 w-full h-[36px] rounded-[8px] active:bg-gray-100 transition-colors">
              ...
              <span className="font-poppins font-medium text-[14px] leading-[20px] text-[#525252]">Quitter le groupe</span>
            </button>
          </div>
        </div>
      </div>
