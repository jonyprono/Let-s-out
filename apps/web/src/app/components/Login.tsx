    ...
    return (
      <div className={authShell}>
        <div className="flex-1 px-6 pt-6 sm:pt-10 flex flex-col overflow-y-auto pb-4 max-w-[400px] mx-auto w-full">
          ...
          <p className={authSubtitle}>Rejoignez des événements près de vous et vivez des expériences inoubliables.</p>

          <label className={authLabel}>Numéro de téléphone</label>
          <div className="flex items-center h-[48px] mb-5 focus-within:border-action-primary focus-within:ring-action-primary transition-all">
            <CountryPicker
              value={country}
              onChange={(c) => { setCountry(c); resetPhone() }}
              className="flex items-center gap-2 h-full px-3 bg-transparent"
            />
            <input
              type="tel"
              inputMode="numeric"
              ...
              className={authPhoneInputFlex}
            />
          </div>

          <label className={authLabel}>Mot de passe</label>
          <div className="relative mb-2">
            ...
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">
              {showPassword ? <ViewIcon width={20} height={20} strokeWidth={1.5} /> : <ViewOffIcon width={20} height={20} strokeWidth={1.5} />}
            </button>
          </div>
          ...
          <div className="mt-auto flex flex-col items-center justify-center gap-4 text-center pb-2">
            <p className="text-[14px] text-gray-600">
              ...
              <button onClick={onSignup} className={authLink}>Inscrivez-vous</button>
            </p>
            <div className="w-[80%] h-px bg-gray-200 my-1" />
            <p className="text-[10px] text-gray-400 max-w-[280px]">
              En continuant, vous acceptez nos <button className={authLink}>Conditions d'Utilisation</button> et notre <button className={authLink}>Politique de Confidentialité</button>
            </p>
          </div>
        </div>
      </div>
