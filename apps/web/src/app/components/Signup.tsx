...
export function Signup({ onBack }: SignupProps) {
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      if (!currentChannel) {
        return toast.error('Veuillez choisir SMS ou Whatsapp.')
      }
          ...
          <div>
            ...
            <div className="flex gap-2 mb-5">
              ...
              <input
                ...
                placeholder=""
                className="flex-1 min-w-0 px-4 py-3.5 border border-[#E5E5E5] rounded-xl text-[15px] focus:outline-none focus:ring-0 focus:border-[#FF9F1C] bg-white text-[#1A1A1A]"
              />
            </div>
              ...
              {['SMS', 'Whatsapp'].map(ch => {
                ...
                return (
                  <button key={ch} onClick={() => setCurrentChannel(val)}
                    className={`flex-1 flex items-center justify-between px-4 py-3.5 border rounded-xl bg-white transition-colors ${isActive ? 'border-[#FF9F1C]' : 'border-[#E5E5E5]'}`}
                  >
