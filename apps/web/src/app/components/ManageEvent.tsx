...
export function ManageEvent() {
  ...
  if (!event) {
    return <div className="w-full h-full flex items-center justify-center bg-[#F9F9F9] dark:bg-[#0a0a0b]">Evénement introuvable</div>;
  }
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#F9F9F9] dark:bg-[#0a0a0b] overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
      ...
      <div className="flex w-full border-b border-gray-200 dark:border-gray-800 shrink-0 bg-white dark:bg-[#1A1A1A]">
        {['details', 'participants', 'cagnotte'].map((tab) => (
          <button
            key={tab}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 mb-6">
        ...
        {/* Co-hosts would go here if we fetched their full profiles, but for now we'll just show the button */}
      ...
      {/* TODO: Proper Friends Search Modal for Co-Host */}
      {showSearchModal && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-end justify-center">
          <div className="bg-white dark:bg-[#1A1A1A] w-full max-w-md h-[80vh] rounded-t-3xl flex flex-col relative">
             ...
             {/* Simple mock list for demonstration. You would hook this up to usersApi.searchUsers() */}
             <div className="p-4 overflow-y-auto flex-1">
                <p className="text-[13px] text-gray-500 mb-4">Recherchez un ami pour l'ajouter comme co-organisateur.</p>
                <div className="flex flex-col gap-3">
                  {/* Fake item for functionality test */}
...
function TabParticipants({ bookings }: { event: any, bookings: any[] }) {
  ...
  // Real participants are those with 'CONFIRMED' status.
        ...
        {selectedUser && (
          <div className="w-full flex flex-col items-center pt-2 pb-6 px-4">
            ...
            <div className="w-full">
              <h4 className="text-[12px] font-semibold text-gray-500 mb-3">Groupes communs</h4>
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-full bg-gray-200 shrink-0" />
                <span className="text-[14px] font-medium text-gray-800 dark:text-gray-200">Nintendo party</span>
              </div>

              <h4 className="text-[12px] font-semibold text-gray-500 mb-3">Actions</h4>
              <div className="flex flex-col gap-1">
                <button className="flex items-center gap-3 p-3 text-[14px] font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl text-left">
  ...
  if (hasPot) {
    return (
      <div className="flex flex-col items-center justify-center py-10 px-4">
         <div className="bg-white dark:bg-[#1A1A1A] w-full rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 text-center">
            <h3 className="text-[18px] font-bold text-[#FF7A00] mb-2">Cagnotte active</h3>
            <p className="text-[14px] text-gray-600 dark:text-gray-400 mb-4">{event.poolDescription || 'Cagnotte de l\'événement'}</p>
            <div className="text-[32px] font-extrabold text-gray-900 dark:text-white">{event.poolTarget} <span className="text-[16px] text-gray-500 font-medium">XOF</span></div>
            <p className="text-[12px] text-gray-500 mt-4">Mode : {event.poolMode}</p>
         </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4">
      ...
      <button 
        ...
        className="w-full py-3.5 rounded-xl border border-gray-200 dark:border-gray-700 text-[14px] font-semibold text-gray-900 dark:text-white flex items-center justify-center gap-2"
      >
